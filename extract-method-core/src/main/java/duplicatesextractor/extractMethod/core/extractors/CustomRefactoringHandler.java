...
import duplicatesextractor.extractMethod.core.utils.PsiUtil;
import duplicatesextractor.extractMethod.core.utils.StringUtil;
...
import static duplicatesextractor.extractMethod.core.utils.StringUtil.calculateSignature;
import static duplicatesextractor.extractMethod.metrics.MetricCalculator.writeFeaturesToFile;
        ...
        for (Refactoring ref : extractMethodRefactorings) {
            ...
            for (String path : changedSourceJavaFiles.keySet()) {
                if (path.endsWith(sourceLocationInfo.getFilePath())) {
                    PsiMethod dummyMethod = PsiUtil.findMethodBySignature(changedSourceJavaFiles.get(path), StringUtil.calculateSignature(sourceOperation));
